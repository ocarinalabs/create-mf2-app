...
tags: ui, ux, accessibility, performance, design, forms, animations
---
...
- **Implementation preference.** Prefer CSS, avoid main-thread JS-driven animations when possible. Preference: CSS > Web Animations API > JavaScript libraries e.g., [motion](https://www.npmjs.com/package/motion).
...
- **Necessity check.** Only animate when it clarifies cause & effect or when it adds deliberate delight.
...
- **Respect safe areas.** Account for notches & insets with `env(safe-area-inset-*)` variables.
...
- **Non-breaking spaces for glued terms.** Use `&nbsp;` to keep units, shortcuts & names together: `10 MB` → `10&nbsp;MB`, `⌘ + K` → `⌘&nbsp;+&nbsp;K`, `Vercel SDK` → `Vercel&nbsp;SDK`. Use `&#x2060;` for no space.
...
- **Large lists.** Virtualize large lists e.g., [virtua](https://github.com/inokawa/virtua) or `content-visibility: auto`.
...
- **Error messages guide the exit.** Don't just state what went wrong—tell the user how to fix it. Instead of "Invalid API key," say "Your API key is incorrect or expired. Generate a new key in your account settings." The copy & buttons/links should educate & give a clear action.
- **Avoid ambiguity.** Labels are clear & specific. Instead of the button label "Continue" say "Save API Key".
