# create-mf2-app

The easiest way to start a new project with the MF² Stack.

## Getting Started

```bash
npm create mf2-app@latest
# or
yarn create mf2-app
# or
pnpm create mf2-app
```

## What is the MF² Stack?

The **Move F*cking Fast Stack** is an opinionated, full-stack, typesafe web development starter kit created by [Korrect](https://korrect.app).

## What's Included

The MF² Stack comes with everything you need to ship fast:

- ⚡️ **Next.js 15** - React framework with App Router
- 🔷 **TypeScript** - Type safety out of the box
- 🎨 **Tailwind CSS** - Utility-first styling
- 🔥 **Convex** - Real-time database
- 🔐 **Clerk** - Authentication that just works
- 💳 **Polar** - Payments and subscriptions
- 📊 **PostHog** - Analytics and insights
- ✉️ **Resend** - Transactional emails
- 🎯 **Openpanel** - Privacy-first analytics

## Templates

- **Frontend** - Landing pages, marketing sites, documentation
- **Full-stack** - SaaS apps with auth, payments, and real-time features

## CLI Options

```bash
# Specify a project name
npm create mf2-app@latest my-app

# Use a specific package manager
npm create mf2-app@latest my-app --use-pnpm
```

## Philosophy

Stop overthinking. Start shipping. The MF² Stack gives you a production-ready foundation so you can focus on what makes your product unique.

## Community

- [Discord](https://discord.gg/mf2stack)
- [GitHub](https://github.com/korrect-ai/create-mf2-app)
- [Twitter](https://twitter.com/korrect)

## License

MIT © Korrect